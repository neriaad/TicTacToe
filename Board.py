...
class Board:
    ...
    def check_winner(self):
        for i in range(3):
            if self.board[i][0] == self.board[i][1] == self.board[i][2] != '-':
                self.winner = getattr(Shape, self.board[i][0])
                return
            if self.board[0][i] == self.board[1][i] == self.board[2][i] != '-':
                self.winner = getattr(Shape, self.board[0][i])
                return
        if self.board[0][0] == self.board[1][1] == self.board[2][2] != '-':
            self.winner = getattr(Shape, self.board[0][0])
            return
        if self.board[0][2] == self.board[1][1] == self.board[2][0] != '-':
            self.winner = getattr(Shape, self.board[0][2])
            return
    ...
    def enter_move(self, coordinates_input: List[int], shape_turn: Shape):
        self.board[coordinates_input[0]-1][coordinates_input[1]-1] = shape_turn.value
